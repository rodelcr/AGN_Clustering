...
class BerWei02(HaloProfileHOD):
    ...
    __repr_attrs__ = __eq_attrs__ = (
        "log10Mmin_0", "log10Mmin_p", "siglog10M_0", "siglog10M_p", "log10M0_0",
    ...
    def __init__(self, *, mass_def, concentration,
                 ...
                 siglog10M_p=0., log10M0_0=7., log10M0_p=0.,
    ...
    def _Nc(self, M, a):
        ...
        result =  np.heaviside(M-Mmin, 1) #* self._Nc(M, a) # 0 \times nan != 0 # don't include the additional Nc factor bc P1h is defined with N = Nc(1 + Ns ) 
    ...
    def _Ns(self, M, a):
        ...
        result =  np.heaviside(M-Mmin, 1) * (((M)/ M1)**alpha) #* self._Nc(M, a) # 0 \times nan != 0 # don't include the additional Nc factor bc P1h is defined with N = Nc(1 + Ns ) 
        result[np.isnan(result) == True] = 0
