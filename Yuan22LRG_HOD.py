__all__ = ("Yuan22LRG_HOD",)

import numpy as np
from scipy.special import sici, erf, erfc
...
class Yuan22LRG_HOD(HaloProfileHOD):
    ...
    __repr_attrs__ = __eq_attrs__ = (
        "log10Mcut_0", "log10Mcut_p", "siglog10M_0", "siglog10M_p", "kappa_0",
        "kappa_p", "log10M1_0", "log10M1_p", "alpha_0", "alpha_p", "fc_0",
        "fc_p", "bg_0", "bg_p", "bmax_0", "bmax_p", "a_pivot",
        "_is_number_counts", "ns_independent", "mass_def", "concentration",
        "precision_fftlog", "fic")

    def __init__(self, *, mass_def, concentration,
                 log10Mcut_0=12., log10Mcut_p=0., siglog10M_0=0.4,
                 siglog10M_p=0., kappa_0=1.2, kappa_p=0.,
                 log10M1_0=13.3, log10M1_p=0., alpha_0=1.,
                 alpha_p=0., fc_0=1., fc_p=0., fic =1.,
    ...
    def update_parameters(self, *, log10Mcut_0=None, log10Mcut_p=None,
                          siglog10M_0=None, siglog10M_p=None,
                          kappa_0=None, kappa_p=None,
    ...
    def _Nc(self, M, a):
        ...
        return self.fic *  0.5 * (erfc(np.log10(Mcut/M)/(np.sqrt(2)*siglog10M)))

    def _Ns(self, M, a):
        # Number of satellites
        Mcut = 10.**(self.log10Mcut_0 + self.log10Mcut_p * (a - self.a_pivot))
        kappa = self.kappa_0 + self.kappa_p * (a - self.a_pivot)
        M1 = 10.**(self.log10M1_0 + self.log10M1_p * (a - self.a_pivot))
        alpha = self.alpha_0 + self.alpha_p * (a - self.a_pivot)
        result =  np.heaviside(M-(kappa*Mcut), 1) * ((( M-(kappa*Mcut)  )/ M1)**alpha) #* self._Nc(M, a) # 0 \times nan != 0 # don't include the additional Nc factor bc P1h is defined with N = Nc(1 + Ns ) 
