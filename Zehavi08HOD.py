...
class Zehavi08HOD(HaloProfileHOD):
    ...
    def _Nc(self, M, a):
        ...
        result = np.exp(-Mmin/M) #np.heaviside(M-Mmin, 1) #* self._Nc(M, a) # 0 \times nan != 0 # don't include the additional Nc factor bc P1h is defined with N = Nc(1 + Ns ) 
    ...
    def _Ns(self, M, a):
        ...
        result =  (((M)/ M1)**alpha) #* self._Nc(M, a) # 0 \times nan != 0 # don't include the additional Nc factor bc P1h is defined with N = Nc(1 + Ns ) 
        result[np.isnan(result) == True] = 0
