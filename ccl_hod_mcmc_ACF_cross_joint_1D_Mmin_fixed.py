...
outfile_subject = str(sys.argv[12])
...
def log_prior_Mmin(sample, hod_str = 'nicola20'):
    #Adding a uniform prior ftom the reddened AGN HOD solution on the M_1 parameter to test how it affects the T1 analysis of the 1halo term 
    ...
    if hod_str=="zehavi08":
        ...
        if 12.46 < M_min < 12.48 and min_M < M_1 < max_M and 0.0 < alpha < 2.0:
            return 0.0
        else:
            return -np.inf
    ...
    if hod_str=="nicola20" or hod_str=="zheng07":
        ...
        max_M = 16.95
        min_M = 9.0
        if 12.46 < M_min < 12.48 and min_M < M_0 < max_M and 0.0 < alpha < 2.0 and 0 < sig_logm < 2 and min_M < M_1 < max_M and (((M_0/M_min) -1) >= 0):
            return 0.0
        else:
            return -np.inf
        ...
        print("for HODs I expect zheng07, zehavi08, or nicola20")
...
def log_probability_i(sample, thetas, inputACF, inputerrs, zgrid = zgrid_list[0], dNdz = dNdz_list[0], LRG_solution = LRG_solution_list[0], zs2 = zgrid_LRG_list[0], dNdz2 = dNdz_LRG_list[0]):
    lp = log_prior_Mmin(sample, hod_str = whichHOD)
...
filename = f"../{outfile_subject}_ACF_HOD_Mmin_12pt47_fixed_"+whichHOD+"_MCMC_0417_"+str(steps)+"_steps.h5"
