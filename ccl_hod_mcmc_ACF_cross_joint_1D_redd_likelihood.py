...
def log_prior_redd(sample, hod_str = 'nicola20'):
    ...
    if hod_str=="zehavi08":
        ...
        if min_M < M_min < 15. and M1_min < M_1 < M1_max and 0.0 < alpha < 2.0:
            return 0.0
        else:
            return -np.inf
    ...
    if hod_str=="nicola20" or hod_str=="zheng07":
        ...
        max_M = 16.95
        min_M = 9.0
        if min_M < M_min < 15. and min_M < M_0 < max_M and 0.0 < alpha < 2.0 and 0 < sig_logm < 2 and M1_min < M_1 < M1_max and (((M_0/M_min) -1) >= 0):
            return 0.0
        else:
            return -np.inf
...
def log_probability_i(sample, thetas, inputACF, inputerrs, zgrid = zgrid_list[0], dNdz = dNdz_list[0], LRG_solution = LRG_solution_list[0], zs2 = zgrid_LRG_list[0], dNdz2 = dNdz_LRG_list[0]):
    lp = log_prior_redd(sample, hod_str = whichHOD)
...
filename = f"../{outfile_subject}_ACF_HOD_redd_prior_"+whichHOD+"_MCMC_0226_"+str(steps)+"_steps.h5"
