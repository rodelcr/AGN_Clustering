...
def log_prior_Mmin(sample, hod_str = 'nicola20'):
    ...
    if hod_str=="zehavi08":
        ...
        if min_M < M_min < max_M and min_M < M_1 < max_M and -2 < alpha < 4.0:
            return 0.0
        else:
            return -np.inf
    ...
    if hod_str=="nicola20" or hod_str=="zheng07":
        ...
        max_M = 16.95
        min_M = 9.0
        if 12.46 < M_min < 12.48 and min_M < M_0 < max_M and -2 < alpha < 4.0 and 0 < sig_logm < 2 and min_M < M_1 < max_M and (((M_0/M_min) -1) >= 0):
            return 0.0
        else:
            return -np.inf
...
filename = f"../{outfile_subject}_ACF_HOD_wide_prior_"+whichHOD+"_MCMC_0417_"+str(steps)+"_steps.h5"
